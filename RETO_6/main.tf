...
# --- web-server (con httpd instalado via user_data) ---
  ...
  user_data = <<-EOF
    #!/bin/bash
    # Registrar log de depuración
    exec > /var/log/ssm-setup.log 2>&1
    echo "--- Iniciando configuración SSM ---"
    
    # Asegurar que el agente SSM está instalado y activo
    systemctl enable --now amazon-ssm-agent
    systemctl restart amazon-ssm-agent
    
    # Esperar conexión a internet antes de seguir
    ...
    dnf update -y
    dnf install -y httpd samba
    systemctl enable --now httpd
    systemctl enable --now smb
    echo "<h1>Web Server - Reto 6 SSM</h1>" > /var/www/html/index.html
    
    # Forzar reinicio final del agente para asegurar registro
    systemctl restart amazon-ssm-agent
